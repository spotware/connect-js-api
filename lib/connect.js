...
Connect.prototype.start = function () {
    ...
    var transport = new protostream.Stream({
...
Connect.prototype.createMessage = function (payloadType, params) {
    var Message = this.protoMessagesCommon.getMessageByPayloadType(payloadType) || this.protoMessagesOpenApi.getMessageByPayloadType(payloadType);
    return new Message(params);
};

Connect.prototype.wrap = function (payloadType, msg) {
    var ProtoMessage = this.protoMessagesCommon.getMessageByName('ProtoMessage');
...
Connect.prototype.sendGuaranteedCommand = function (payloadType, params) {
    var msg = this.createMessage(payloadType, params);
    var wrap = this.wrap(payloadType, msg);
    ...
    if (this.isConnected()) {
        this.sendCommand(wrap.toBuffer());
    }
    return command;
};
...
Connect.prototype.findCommand = function (clientMsgId) {
    return this.openCommands.find(function (command) {
        return command.message.clientMsgId === clientMsgId;
    });
};
